...
const readFilesInDirectory = (dir: string): string[] =>
  fs.readdirSync(dir).reduce((files: string[], file: string) => {
    ...
    return isDirectory
      ? [...files, ...readFilesInDirectory(name)]
      : [...files, name];
  }, []);
...
export const extractRoutesToPrerender = () => {
  ...
  const routes = extractRoutesFromFileNames(
    readFilesInDirectory('./src/app/pages')
  );
  ...
  const contentSlugs = extractRoutesFromFileNames(
    readFilesInDirectory('./src/content')
  );
  // for our :slug route we replace the param with the actual content slug
  const slugRoutes = contentSlugs.map((contentSlug) =>
    routes[slugRouteIndex].replace(':slug', contentSlug)
  );
